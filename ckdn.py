...
class ResNet(nn.Module):
    ...
    def forward_test(self, rest , dist , extract=False):

        rest1 = self.qse_2(self.maxpool(self.qse_1(rest)))
        dist1 = self.dte_2(self.maxpool(self.dte_1(dist)))
        x = rest1 - dist1
        x = self.csp(x)
        x = self.avgpool(x)
        dr = torch.flatten(x, 1) 
        dr = torch.sigmoid(self.fc_(dr))
        return dr
    ...
    def extract_DTE_features(self, img): # 7.7.2023

        dist1 = self.dte_2(self.maxpool(self.dte_1(img)))
        x =  dist1
        x = self.avgpool(x)
        dr = torch.flatten(x, 1) 
        return dr
    ...
    def extract_QSE_features(self, rest): # 8.18.2023

        rest1 = self.qse_2(self.maxpool(self.qse_1(rest)))
        x =  rest1
        x = self.avgpool(x)
        qse_feature = torch.flatten(x, 1) 
        return qse_feature
    

    def extract_QSE_after_minus_features(self, rest , dist ): # 8.18.2023  # dist , ref
    ...
    def extract_last_features(self, rest , dist):  # 9.22.2023  # dist , ref
